namespace RFHandheld
{
    ...
    [Activity(Label = "MoveActivity", Theme = "@style/AppTheme")]
    public class MoveActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.move);

            // ListView to display input
            moveList = FindViewById<ListView>(Resource.Id.list);
            ...
            // Move Function
            Move = FindViewById<Button>(Resource.Id.btApply);
            Move.Click += (sender, e) =>
            {
                OnMoveFunction();
            };
        ...
        public void OnReceivedRFID(List<RfidInfo> data)
        {
            ...
            {
                // Same Location or RFID scan first
        ...
        public async void OnMoveFunction()
        {
            if (RFID != string.Empty && Location != string.Empty)
            {
                Move.Enabled = false;
                Move.Text = "Move request processing...Please wait...";
                var result = await API.MOVE(RFID, Location);
                if (result == "Move - OK")
                {
                    OnClearView();
                }
